...
Multi-persona code review run after implementation. Each subagent audits only the **changed code and its immediate dependents** — not the entire codebase.

## Output

After all subagents complete, write a single consolidated report to the plan file in [docs/exec-plans/active/](docs/exec-plans/active/).

The report **must** include the following sections:

1. A updated **Files changed** section listing every file where findings were flagged (including immediate dependents). This acts as a lightweight index for future planning.

2. **Flag fixed items** — In the audit report mark each finding that was fixed with a `[FIXED]` prefix so it is visually distinct from unresolved items.

3. **Append a fixes summary** — Add an `## Audit Fixes` section at the end of `implementation.md` containing:
...
4. **Leave unresolved items as-is** — Any audit findings intentionally deferred or accepted as-is should remain unmarked in the audit report. Add a brief note in the fixes summary explaining why they were not addressed.

5. **Update testing checklist** — If any audit fixes changed user-facing behavior, add corresponding `- [ ]` test items to `docs/testing-checklist.md`. Each item should describe the expected observable behavior, not the implementation detail.
